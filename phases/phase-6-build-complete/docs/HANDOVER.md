...
### HPA Configuration
- Course Agent service: Scales based on CPU utilization (target: 70%)
- Frontend service: Scales based on CPU utilization (target: 70%)

### Dapr Resilience Policies
- Retry policies with constant back-off for Kafka pub/sub operations
- Circuit breakers for service-to-service calls to prevent cascading failures
